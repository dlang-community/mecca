...
struct CascadingTimeQueue(T, size_t numBins, size_t numLevels) {
    ...
    U64Denominator resolutionDenom;
    size_t offset;
    version(unittest) {
        ulong[numLevels] stats;
    }
    LinkedSet!T[numBins][numLevels] bins;
    ...
    void open(long resolutionCycles, TscTimePoint startTime) {
        ...
        this.resolutionDenom = U64Denominator(resolutionCycles);
        this.offset = 0;
        version (unittest) {
            this.stats[] = 0;
        }
    ...
    @("notrace") private bool reinsert(T entry) {
    ...
    @("notrace") static void discard(T entry) {
        LinkedSet!T.discard(entry);
    }
    ...
    @("notrace") private void cascadeNextLevel(size_t level)() {
